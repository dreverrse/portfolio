  ...
  const submit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!cleanName || cleanName.length > 30) {
        setError(t("guestbook.errName"));
        return;
      }
      if (!cleanMessage || cleanMessage.length > 500) {
        setError(t("guestbook.errMessage"));
        return;
      }
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data?.error || t("guestbook.errSend"));

        setEntries((prev) => [data.entry, ...prev]);
        setMessage("");
      } catch (err) {
        setError(err instanceof Error ? err.message : t("guestbook.errSend"));
      } finally {
        setSubmitting(false);
      }
    },
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center mb-3">
          ...
          <div className="flex-1">
            ...
            >
              ...
              {submitting ? t("guestbook.sending") : t("guestbook.send")}
            </button>
          </div>
        </div>
        <textarea
      ...
      <div className="mt-8 space-y-4">
        ...
        {!loading && entries.length === 0 && (
          <div className="text-center py-16 rounded-xl border border-border bg-card/30">
            ...
            <p className="text-muted text-lg">{t("guestbook.empty")}</p>
            <p className="text-sm text-muted/60 mt-2">
              {t("guestbook.emptySub")}
            </p>
          </div>
        )}
